...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'image_width',
            default_value='1920',
            description='图像宽度（像素），1920',
        ),
        DeclareLaunchArgument(
            'hfov_deg',
            default_value='90.0',
            description='水平视场角（度），90.0',
        ),
        DeclareLaunchArgument(
            'kp',
            default_value='0.1',
            description='比例系数（检测像素偏移 -> yaw误差后再乘kp），0.1',
        ),
        DeclareLaunchArgument(
            'sign',
            default_value='1.0',
            description='方向因子（若方向反了改为 1.0；默认按常见PX4/NED与像素方向相反设置），1.0',
        ),
        DeclareLaunchArgument(
            'center_deadband_px',
            default_value='1',
            description='画面中心死区（像素），小于该偏差不再调整yaw以减小抖动，1',
        ),
        DeclareLaunchArgument(
            'error_lowpass_alpha',
            default_value='0.6',
            description='yaw误差低通滤波系数alpha(0~1)，越大越跟得紧但越抖，0.6',
        ),
        DeclareLaunchArgument(
            'center_weight',
            default_value='0.6',
            description='选目标时“靠近中心”的权重，越大越不容易跳目标，0.6',
        ),
        DeclareLaunchArgument(
            'min_score',
            default_value='0.25',
            description='最小置信度，低于该值的检测将忽略，0.25',
        ),
        DeclareLaunchArgument(
            'hover_x',
            default_value='3.0',
            description='无人机2悬停X（NED/本地坐标，单位m），3.0',
        ),
        DeclareLaunchArgument(
            'hover_y',
            default_value='0.0',
            description='无人机2悬停Y（单位m），0.0',
        ),
        DeclareLaunchArgument(
            'hover_z',
            default_value='-5.0',
            description='无人机2悬停Z（NED向下为负，单位m），-5.0',
        ),
        DeclareLaunchArgument(
            'scan_rate_rad_s',
            default_value='0.8',
            description='目标丢失时原地自转角速度（rad/s），0.8',
        ),
        DeclareLaunchArgument(
            'lost_timeout_s',
            default_value='0.35',
            description='超过该时间未检测到目标则认为丢失并恢复自转（秒），0.35',
        ),
        DeclareLaunchArgument(
            'max_yaw_step_rad',
            default_value='0.25',
            description='每个控制周期最大偏航步长（rad），0.25',
        ),
        Node(
